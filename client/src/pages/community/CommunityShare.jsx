...
const CommunityShare = () => {
  ...
  const [form, setForm] = useState({ name: '', category: 'cooked', quantity: '', address: '', lat: null, lng: null, expiresAt: '', claimedByLimit: 1 });
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      ...
      () => console.log('Location optional for community feed'),
  ...
  useEffect(() => { fetchShares(); }, [location.lat, location.lng, radius]);
  ...
  const handlePostSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ name: '', category: 'cooked', quantity: '', address: '', lat: null, lng: null, expiresAt: '', claimedByLimit: 1 });
  ...
  const handleClaim = async (id) => {
    try {
      ...
      alert('Tapped to claim! Arrange pickup with the sharer.');
      fetchShares();
    } catch (err) { alert(err.response?.data?.message || 'Failed to claim'); }
  };

  const handleGPS = () => {
    if (!location.lat) return alert('GPS still loading, try again in a moment...');
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-1/3">
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 sticky top-24">
            ...
            <p className="text-sm text-text/60 mb-6">Have extra groceries or cooked meals? Post them for locals to claim.</p>
            <form onSubmit={handlePostSubmit} className="space-y-4">
              <input type="text" required placeholder="What are you sharing? (e.g. 5x Apples)" value={form.name} onChange={e=>setForm({...form, name: e.target.value})} className="w-full px-4 py-3 bg-gray-50 rounded-xl outline-none" />
              <div className="grid grid-cols-2 gap-3">
                <select value={form.category} onChange={e=>setForm({...form, category: e.target.value})} className="w-full px-4 py-3 bg-gray-50 rounded-xl outline-none text-sm">
                  <option value="cooked">Cooked Food</option>
                  <option value="produce">Fresh Produce</option>
                  <option value="packaged">Packaged</option>
                </select>
                <input type="number" required placeholder="Qty" value={form.quantity} onChange={e=>setForm({...form, quantity: e.target.value})} className="w-full px-4 py-3 bg-gray-50 rounded-xl outline-none" />
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-text/40 mb-1 ml-1">Photo</label>
                <input type="file" accept="image/*" onChange={e => { setImage(e.target.files[0]); setPreview(URL.createObjectURL(e.target.files[0])); }} className="w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-gray-100 file:text-text cursor-pointer" />
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" min="1" required value={form.claimedByLimit} onChange={e=>setForm({...form, claimedByLimit: e.target.value})} className="w-full px-4 py-3 bg-gray-50 rounded-xl outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-text/40 mb-1 ml-1">Expiry</label>
                  <input type="datetime-local" required value={form.expiresAt} onChange={e=>setForm({...form, expiresAt: e.target.value})} className="w-full px-4 py-3 bg-gray-50 rounded-xl outline-none text-sm" />
                </div>
              </div>
              <div>
                <div className="flex justify-between items-end mb-1">
                  <label className="block text-xs font-bold uppercase tracking-wider text-text/40 ml-1">Pickup Location *</label>
                  <button type="button" onClick={handleGPS} className="text-xs text-primary font-bold flex items-center gap-1">
                    📍 Use GPS {locationMode === 'gps' && <span className="text-green-600">✓</span>}
                  </button>
                </div>
                <input type="text" required placeholder="General neighborhood or address" value={form.address} onChange={e=>setForm({...form, address: e.target.value})} className="w-full px-4 py-3 bg-gray-50 rounded-xl outline-none" />
                {locationMode === 'gps' && form.lat && <p className="text-[10px] text-green-600 font-bold ml-1 mt-1">✅ GPS coordinates bound</p>}
                {locationMode === 'manual' && <p className="text-[10px] text-text/40 ml-1 mt-1">Tip: Use GPS for map pin accuracy</p>}
              </div>
              <button type="submit" disabled={submitting} className="w-full py-3 bg-black text-white font-bold rounded-xl shadow-md hover:bg-gray-800 transition disabled:opacity-50">
        ...
        <div className="lg:w-2/3">
          ...
          <div className="space-y-4">
            {loading ? <p className="text-center py-20 text-text/50">Loading local items...</p> :
             !location.lat ? <p className="text-center py-20 text-yellow-600 font-bold bg-yellow-50 rounded-2xl p-8">📍 Allow location access to view nearby shares.</p> :
             shares.length === 0 ? (
               <div className="bg-white rounded-3xl border border-dashed border-gray-200 p-12 text-center text-text/50">
                 <span className="text-4xl block mb-4">🏠</span>
                 <p className="font-medium">No community shares within {radius}km.</p>
