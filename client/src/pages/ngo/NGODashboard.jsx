...
const NGODashboard = () => {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        console.log('Geo denied on NGODashboard');
        setLocationReady(true); // still unblock UI
      },
  ...
  useEffect(() => {
    if (location.lat && activeTab === 'nearby') fetchData();
  }, [location.lat]);
  ...
  const handleScanQR = async (token) => {
    ...
    try {
      ...
      if (action === 'pickup_confirmed') alert('✅ Pickup confirmed! Food is now in transit.');
      else if (action === 'delivery_confirmed') alert('✅ Delivery confirmed! Great work!');
      fetchData();
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-2 mb-6">
        <button onClick={() => setActiveTab('nearby')} className={`px-6 py-2.5 rounded-xl font-medium transition ${activeTab === 'nearby' ? 'bg-primary text-white shadow-md' : 'bg-white text-text/60 hover:bg-gray-50'}`}>Nearby Donations</button>
        <button onClick={() => setActiveTab('missions')} className={`px-6 py-2.5 rounded-xl font-medium transition ${activeTab === 'missions' ? 'bg-primary text-white shadow-md' : 'bg-white text-text/60 hover:bg-gray-50'}`}>Active Missions 🚀</button>
      </div>
          ...
          <>
            {!location.lat && (
              <p className="p-4 bg-yellow-50 text-yellow-800 rounded-xl font-medium">
                📍 Location access denied — enable GPS to find nearby donations.
              </p>
            )}
            {location.lat && donations.length === 0 && (
                ...
                <div key={m._id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-6 hover:shadow-md transition">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 text-sm text-text/80 mb-2">
                      <span className="text-xl">🏾d</span><span>From: {m.pickupAddress}</span>
                    </div>
