...
const ConsumerMarketplace = () => {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => console.log('Marketplace geo error ignored', err),
    ...
    const handlePriceUpdate = (data) => {
      setProducts(prev => prev.map(p => {
        if (p._id === data.productId) {
          return { ...p, finalPrice: data.finalPrice, discountPercent: data.discountPercent, urgentBadge: data.urgentBadge };
        }
        return p;
      }));
    };
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => setUrgentOnly(!urgentOnly)} className={`flex items-center gap-2 px-4 py-2 rounded-full font-bold text-sm transition border ${urgentOnly ? 'bg-red-50 text-red-700 border-red-200' : 'bg-transparent text-text/60 border-gray-200 hover:bg-gray-50'}`}>
            ...
            <button onClick={() => setIsCartOpen(true)} className="relative p-2 bg-primary/10 text-primary rounded-full hover:bg-primary/20 transition">
              ...
              {items.length > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full font-bold shadow-sm">{items.length}</span>}
            </button>
          </div>
        </div>
            ...
            {products.map(p => (
              <div key={p._id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col group hover:shadow-md transition">
                <div className="relative h-48 bg-gray-50">
                  ...
                  {p.discountPercent > 0 && <div className="absolute top-2 right-2 bg-accent text-white font-black text-xs px-2.5 py-1 rounded-full shadow-md">{p.discountPercent}% OFF</div>}
                </div>
                ...
                <div className="p-4 flex flex-col flex-1 justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-medium text-text/60">{p.quantity} avg. stock left</span>
                    <button onClick={() => addItem(p)} className="ml-auto w-10 h-10 rounded-full bg-primary/10 text-primary hover:bg-primary hover:text-white transition font-black text-xl flex items-center justify-center shadow-sm">
                      +
                    </button>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-full flex flex-col animate-slide-in-right shadow-2xl border-l border-gray-100">
            ...
            <div className="p-5 border-t border-gray-100 bg-gray-50/50">
              ...
              <button onClick={handleCheckout} disabled={items.length === 0} className="w-full py-4 bg-black text-white rounded-xl font-bold shadow-md hover:bg-gray-800 disabled:opacity-50 transition transform active:scale-[0.98]">
                Proceed to Payment
              </button>
