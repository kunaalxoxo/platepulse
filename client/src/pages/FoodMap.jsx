...
const FoodMap = () => {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        console.warn('Geo denied — using India default center');
        setLocationReady(true); // Still allow map to load with default
      },
  ...
  // Fetch markers once GPS is ready (or denied with default)
  useEffect(() => {
    if (locationReady) fetchMarkers();
  }, [locationReady]);
  ...
  const getDonationColor = (expiresAt) => {
    ...
    if (hrsLeft < 6) return '#F44336';
    if (hrsLeft < 24) return '#FFC107';
    return '#4CAF50';
  };

  if (loadError) return <div className="bg-red-50 p-10 text-center m-10 text-red-600 font-bold rounded-2xl">Error loading Google Maps. Check VITE_GOOGLE_MAPS_API_KEY env var.</div>;
  ...
  return (
    <div className="relative" style={{ height: 'calc(100vh - 64px)' }}>
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 md:hidden">
        <button onClick={() => setMobileFilterOpen(true)} className="bg-white px-6 py-2.5 rounded-full shadow-lg font-bold text-sm border border-gray-100 text-black whitespace-nowrap">🔍 Filter Map</button>
      </div>

      <div className={`fixed inset-0 bg-black/50 z-20 md:bg-transparent md:pointer-events-none md:absolute flex flex-col justify-end md:justify-start transition-opacity duration-300 ${mobileFilterOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none md:opacity-100 md:pointer-events-auto'}`}
        onClick={(e) => e.target === e.currentTarget && setMobileFilterOpen(false)}>
        <div className={`w-full md:w-80 bg-white h-[75vh] md:h-full md:border-r border-gray-100 shadow-2xl md:shadow-md flex flex-col transform transition-transform duration-300 md:translate-y-0 relative pointer-events-auto rounded-t-3xl md:rounded-none ${mobileFilterOpen ? 'translate-y-0' : 'translate-y-full md:translate-y-0'}`}>
            ...
            <div>
              ...
              <div className="flex gap-2 bg-gray-50 p-1 rounded-xl">
                {[['all','All'],['24h','<24h'],['6h','<6h'],['1h','<1h']].map(([v,l]) => (
                  <button key={v} onClick={()=>setExpiryFilter(v)} className={`flex-1 py-1.5 text-xs font-bold rounded-lg ${expiryFilter===v?'bg-white shadow-sm text-black':'text-text/50'}`}>{l}</button>
                ))}
              </div>
            </div>
            <div>
      ...
      <div className="w-full h-full md:pl-80 bg-gray-100">
        <GoogleMap mapContainerStyle={mapContainerStyle} center={location} zoom={13}
          options={{ zoomControl: true, mapTypeControl: false, streetViewControl: false, fullscreenControl: true }}>
          ...
          {selectedMarker && (
            ...
            >
              <div className="w-48 sm:w-60 font-sans p-1">
                {selectedMarker.payload.image && <img src={selectedMarker.payload.image} className="w-full h-28 object-cover rounded-lg mb-3 shadow-sm bg-gray-50" />}
                ...
                {selectedMarker.type === 'donation' && <button onClick={() => navigate('/dashboard')} className="w-full py-2 bg-primary text-white font-bold text-sm rounded-lg">View Delivery Mission</button>}
                {selectedMarker.type === 'product' && <button onClick={() => navigate('/marketplace')} className="w-full py-2 bg-blue-600 text-white font-bold text-sm rounded-lg">Buy in Marketplace (₹{selectedMarker.payload.finalPrice})</button>}
                {selectedMarker.type === 'community' && <button onClick={() => navigate('/community')} className="w-full py-2 bg-purple-600 text-white font-bold text-sm rounded-lg">Claim Portion ({selectedMarker.payload.claimedCount}/{selectedMarker.payload.claimedByLimit})</button>}
                {selectedMarker.type === 'waste' && <p className="w-full py-2 bg-gray-800 text-white font-bold text-sm rounded-lg text-center mt-3">Waste Plant</p>}
              </div>
            </InfoWindowF>
          )}
        </GoogleMap>
