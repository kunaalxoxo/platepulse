...
const sendVerificationOTP = async (email, name, otp) => {
  try {
    ...
    await transporter.sendMail({
      from: `"PlatePulse" <${env.EMAIL_USER}>`,
      to: email,
...
const sendPasswordResetOTP = async (email, name, otp) => {
  try {
    if (!env.EMAIL_USER) {
      logger.warn('Email not configured — skipping reset email');
...
const sendMatchNotification = async (email, name, donationName, distance) => {
  try {
    if (!env.EMAIL_USER) {
      logger.warn('Email not configured — skipping match notification');
      return;
    }
    const html = `
      <div style="${baseStyle}">
        <div style="${cardStyle}">
          ...
          <div style="padding:32px;">
            <h2 style="margin:0 0 8px;color:#2C2C2C;font-size:20px;">Hi ${name},</h2>
            <p style="color:#666;font-size:14px;line-height:1.6;">
              A new food donation <strong>"${donationName}"</strong> is available just
              <strong>${distance}km</strong> away from you.
            </p>
            <div style="text-align:center;margin:24px 0;">
              <a href="${env.CLIENT_URL}/map"
                 style="display:inline-block;background:#2E7D32;color:#fff;padding:12px 32px;
          ...
          <div style="background:#F5F7F6;padding:16px 32px;text-align:center;border-top:1px solid #eee;">
            <p style="margin:0;color:#999;font-size:11px;">You're receiving this because you enabled donation alerts.</p>
          </div>
        </div>
      </div>
    `;
